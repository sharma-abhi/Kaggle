...
```{r}
# deleting all rows having atleast one NA
train_wo_na <- train[complete.cases(train),] 
dim(train_wo_na)
dim(train)
```
It's evident that we cannot simply delete the NA rows as we are left with only 
20% data. 

We approach this in a column-to-column basis.
```{r}
summary(train)
